...
- It is assumed that document "status" is either being assessed by a 3rd party, or handled elsewhere, and therefore there is no way to manually "verify" or "reject" a document in this app.  

- React-router's BrowserRouter was used for speed of setup, as opposed to file-based routing. This is discussed further in the improvements section.  
...
- Better file validation is needed. Currently we are only enforcing a max file size, and doc types via magic byte sniffing. There could be a multitude of checks including strict file size limits per type, visual checks for images to prevent blurry pictures from being accepted, and resolution limits.  
...
- The UI is bare bones. Improved styling, component placement, verbiage, error display, etc. would need to happen for a production ready app.  
